...
while resume_token:
	try:
		if resume_token == 'FIRST':
			quota_query = quota_api.list_quota_quotas(resolve_names=True)
		elif resume_token:
			quota_query = quota_api.list_quota_quotas(resume=resume_token)

		resume_token = quota_query.resume

	except rest.ApiException as err:
		print(f"Api Exception: {err}")

	for quota in quota_query.quotas:
		cur = {
			'type': quota.type,
			'path': quota.path,
